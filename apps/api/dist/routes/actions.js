...
import { actionIdParamsSchema, } from "../schemas/actions.js";
...
router.post("/plan/actions/:actionId/mark-paid", validateParams(actionIdParamsSchema), async (c) => {
    const userId = requireUserId(c);
    const withRls = requireWithRls(c);
    const { actionId } = c.get("validatedParams");
    const [latestPlan] = await withRls((tx) => tx
        .select()
        .from(plansTable)
    ...
    const snapshot = latestPlan.snapshotJson;
    ...
    const [updated] = await withRls((tx) => tx
        .update(plansTable)
        .set({
        snapshotJson: sql `jsonb_set(${plansTable.snapshotJson}, ${sql.raw(`'${jsonPath}'`)}, to_jsonb(NOW()), false)`,
    })
