 ...
 */
function getTargetDueDate(card, referenceDate) {
 ...
 */
function getTargetStatementCloseDate(card) {
 ...
 */
function buildMinimumActions(cards, referenceDate) {
    const actions = [];
 ...
 */
function distributeSnowball(cards, extraCashCents, byCardId) {
 ...
 * Updates existing BY_DUE_DATE actions with additional amount; does not exceed balance.
 */
function distributeAvalanche(cards, extraCashCents, byCardId) {
 ...
 */
function distributeUtilization(cards, extraCashCents, actions) {
    const withUtilAndClose = cards
        ...
        .filter((x) => x.closeDate != null && x.util > 30);
    ...
    for (const { card, closeDate } of sorted) {
        ...
        const payBeforeCents = Math.min(Math.max(0, balanceAfterMin - targetBalanceCents), balanceAfterMin, remaining);
        if (payBeforeCents > 0) {
 ...
 */
export function allocatePayments(cards, availableCashCents, strategy, referenceDate) {
    const ref = referenceDate ?? new Date();
    const totalMinimumsCents = cards.reduce((sum, c) => sum + (c.minimumDueCents ?? 0), 0);
