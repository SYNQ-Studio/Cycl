 ...
 */
export function validateConstraints(cards, availableCashCents) {
    const totalMinimumCents = cards.reduce((sum, card) => sum + (card.minimumDueCents ?? 0), 0);
 ...
 */
function buildSuggestions(shortfallCents) {
    const result = [
        {
            kind: "increase_cash",
        ...
        {
            kind: "reduce_cards",
            message: "Reduce the number of cards or lower minimum payments so total minimums do not exceed available cash.",
        },
    ];
    return result;
}
